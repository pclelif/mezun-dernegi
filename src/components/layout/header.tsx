  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/10 bg-white/95 backdrop-blur-sm">
      <div className="mx-auto flex min-h-20 w-[min(100%-2rem,75rem)] items-center justify-between gap-5 md:w-[min(100%-4rem,75rem)]">
        <Link href="/" className="flex max-w-[19rem] items-center gap-3 rounded-sm text-xs font-bold leading-4 tracking-tight text-zinc-950 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-red-600" aria-label={`${associationName} ana sayfa`}>
      ...
      <nav id="mobile-navigation" className={`${isOpen ? "flex" : "hidden"} fixed inset-x-0 top-20 h-[calc(100dvh-5rem)] flex-col overflow-y-auto border-t border-zinc-200 bg-white px-4 py-5 xl:hidden`} aria-label="Mobil menü">
        {navigation.map((item, index) => {
          if (!item.children) {
