...
export function DataRecipients() {
  return <>
    <ul className={legalListClass}>
      <li><strong>Vercel — site barındırma:</strong> siteye erişirken kullanılan IP adresi, istek zamanı, URL ve tarayıcı/istek bilgileri; sayfaların sunulması ve teknik güvenlik amacıyla.</li>
      <li><strong>Supabase — veritabanı, yönetici oturumu ve dosyalar:</strong> iletişim formu kayıtları, yönetici hesap/oturum verileri ve yayımlanan görseller; kayıt saklama, yetkilendirme ve içerik sunumu amacıyla. Projenin veritabanı bölgesi Frankfurt, Almanya’dır. Bu bölge seçimi destek, yedekleme ve alt hizmet sağlayıcı işlemlerinin yalnızca Almanya’da gerçekleştiği anlamına gelmez.</li>
      <li><strong>Google / Gmail — e-posta:</strong> dernek adreslerine gönderilen e-postalar, gönderen/alıcı bilgileri ve ekleri; yazışma ve üyelik başvurusunun iletilmesi amacıyla. Adli sicil belgesi e-postayla gönderildiğinde bu aktarımın kapsamına girer.</li>
      <li><strong>Google Maps — harita:</strong> iletişim sayfasındaki gömülü harita yüklendiğinde IP adresi, tarayıcı ve istek bilgileri Google’a iletilebilir. Harita hizmeti Google Analytics’ten ayrıdır.</li>
      <li><strong>Bankalar ve yetkili kamu kurumları:</strong> ödeme işlemleri ve kanuni bildirimler için gerekli kimlik, üyelik ve işlem bilgileri; ilgili işlemin ve kanuni yükümlülüğün sınırları içinde.</li>
    </ul>
...
export function RetentionDetails() {
  return <>
    <ul className={legalListClass}>
      ...
      <li><strong>Görseller ve teknik kayıtlar:</strong> ilgili yayın amacı, geçerli işleme şartı, güvenlik olayının takibi ve hizmet sağlayıcının kayıt/yedek döngüsü ayrı ayrı dikkate alınır. Yayının sona ermesi ve hukuki sebebin ortadan kalkması saklama ihtiyacının yeniden değerlendirilmesini gerektirir.</li>
    </ul>
