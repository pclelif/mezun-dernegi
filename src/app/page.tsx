    ...
    <>
      ...
      <section className="mx-auto w-[min(100%-2rem,75rem)] py-14 md:w-[min(100%-4rem,75rem)] md:py-20">
        <div className="max-w-3xl">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-red-600">Hikâyemiz</p>
          <h2 className="mt-3 text-2xl font-bold tracking-tight text-zinc-950 md:text-3xl">Aynı sıralardan güçlü bir mezun ağına</h2>
          <p className="mt-5 text-base leading-7 text-zinc-600 md:text-lg md:leading-8">Derneğimiz; mezunlarımız arasındaki iletişimi canlı tutmak, dayanışmayı büyütmek ve okulumuza kalıcı katkılar sunmak için çalışır.</p>
          <Link className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-zinc-950" href="/kurumsal/hakkimizda">
            Derneğimizi tanıyın
      ...
      <section className="border-t border-zinc-200 bg-zinc-50 py-12 text-center"><div className="mx-auto w-[min(100%-2rem,48rem)]"><h2 className="text-2xl font-bold text-zinc-950 md:text-3xl">Mezun topluluğumuzun bir parçası olun.</h2><p className="mt-3 leading-7 text-zinc-600">Bağlarımızı güçlendirmek ve çalışmalarımıza katkı sunmak için derneğimize katılın.</p><Link href="/uyelik/basvuru" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-md bg-red-600 px-6 font-semibold text-white">Üyelik başvurusu yap</Link></div></section>
    </>
  );
}
