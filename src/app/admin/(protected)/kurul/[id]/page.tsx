...
export default function EditBoardMemberPage() {
  ...
  const [item, setItem] = useState<DbBoardMember | null>(null);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setItem(data as DbBoardMember);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Kurul üyesi yüklenemedi.");
      }
    }
  ...
  return <BoardMemberForm initial={item} />;
}
