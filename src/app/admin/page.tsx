  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="flex flex-col justify-between rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
          <div>
            <div className="flex items-center justify-between gap-3 border-b border-zinc-100 pb-2.5">
              <div className="flex items-center gap-2">
                <span className="grid size-7 place-items-center rounded-lg bg-red-50 text-red-600">
                  <Bell className="size-3.5" />
                </span>
                <h2 className="text-sm font-bold text-zinc-950">Bildirimler</h2>
              </div>
              {(unread.count ?? 0) > 0 ? (
              ...
              {notifications.length > 0 ? (
                notifications.map((msg) => (
                  <Link
                    key={msg.id}
                    href="/admin/iletisim"
                    className={`block rounded-lg border px-3 py-2.5 transition hover:border-red-200 hover:bg-red-50/40 ${
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <p className="truncate text-xs font-bold text-zinc-900">{msg.name}</p>
                          {!msg.is_read ? (
                            <span className="shrink-0 rounded bg-red-600 px-1.5 py-0.2 text-[9px] font-bold uppercase tracking-wider text-white">
                              Yeni
                            </span>
                          ) : null}
                        </div>
                ...
                <p className="py-4 text-center text-xs text-slate-500">Henüz bildirim bulunmuyor.</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-3 border-t border-zinc-100 pt-2">
            ...
            >
              Tüm İletişim Mesajlarını Görüntüle <ArrowRight className="size-3" />
            </Link>
          </div>
        </section>
        ...
        <section className="flex flex-col justify-between rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
          <div>
            <div className="flex items-center justify-between gap-3 border-b border-zinc-100 pb-2.5">
              <h2 className="text-sm font-bold text-zinc-950">Son Eklenenler</h2>
              <span className="text-[11px] font-medium text-slate-500">Son güncellemeler</span>
            </div>
              ...
              {latestContent.length > 0 ? (
                latestContent.map((item) => (
                  <div
                    ...
                    className="flex items-center justify-between gap-2 rounded-lg border border-zinc-100 bg-slate-50/40 px-3 py-2.5 text-xs"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-medium text-zinc-900">{item.title}</p>
                      <p className="mt-0.5 text-[10px] text-slate-400">{formatTurkishDate(item.created_at)}</p>
                    </div>
                    <span className="shrink-0 rounded bg-zinc-200/70 px-1.5 py-0.5 text-[10px] font-semibold text-zinc-700">
                      {item.type}
                    </span>
                  </div>
                ))
              ) : (
                <p className="py-4 text-center text-xs text-slate-500">Henüz içerik eklenmedi.</p>
              )}
            </div>
          </div>

          <div className="mt-3 border-t border-zinc-100 pt-2 flex gap-3 text-[11px] font-bold text-red-600">
      ...
      <section className="space-y-2.5">
        <h2 className="text-sm font-bold text-zinc-950">İstatistikler</h2>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
          {stats.map(({ label, value, href, icon: Icon }) => (
            <Link
              href={href}
              key={label}
              className="group rounded-xl border border-zinc-200 bg-white p-3.5 shadow-sm transition hover:border-red-200 hover:shadow-md"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="grid size-8 place-items-center rounded-lg bg-red-50 text-red-600 transition group-hover:bg-red-600 group-hover:text-white">
