...
export default function HomePage() {
  return (
    <>
      <section className="flex min-h-[70vh] items-end border-b-8 border-[#EC1C24] bg-[#6C757D] text-white">
        <div className="container-site py-16 md:py-24">
          ...
          <Link href="/uyelik" className="mt-8 inline-flex rounded-full bg-[#EC1C24] px-6 py-3 font-bold text-white">Üye Ol</Link>
        </div>
      </section>
      ...
      <section className="section-space bg-[#F1F3F5]">
        <div className="container-site grid gap-12 lg:grid-cols-2">
          <div><SectionHeading eyebrow="Güncel" title="Duyurular" />{announcements.map((item) => <article className="mb-4 rounded-2xl border-t-4 border-[#EC1C24] bg-white p-6" key={item.slug}><time className="text-sm font-medium text-black">{item.date}</time><h3 className="mt-2 text-xl font-bold">{item.title}</h3></article>)}</div>
          <div><SectionHeading eyebrow="Takvim" title="Yaklaşan etkinlikler" />{events.map((item) => <article className="mb-4 rounded-2xl border-l-4 border-[#EC1C24] bg-white p-6" key={item.slug}><time className="text-sm text-[#EC1C24]">{item.date}</time><h3 className="mt-2 text-xl font-bold">{item.title}</h3><p className="mt-2 text-black">{item.location}</p></article>)}</div>
        </div>
      </section>
