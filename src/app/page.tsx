...
export default function HomePage() {
  return (
    <>
      <section className="flex min-h-[70vh] items-end bg-[#6C757D] text-white">
      ...
      <section className="section-space bg-[#F1F3F5]">
        <div className="container-site grid gap-12 lg:grid-cols-2">
          ...
          <div><SectionHeading eyebrow="Takvim" title="Yaklaşan etkinlikler" />{events.map((item) => <article className="mb-4 rounded-2xl border-l-4 border-[#EC1C24] bg-white p-6" key={item.slug}><time className="text-sm text-[#EC1C24]">{item.date}</time><h3 className="mt-2 text-xl font-bold">{item.title}</h3><p className="mt-2 text-[#6C757D]">{item.location}</p></article>)}</div>
        </div>
      </section>
      ...
      <section className="section-space bg-[#6C757D] text-center text-white">
        <div className="container-site"><h2 className="text-3xl font-bold md:text-5xl">Sen de bu hikâyenin bir parçası ol.</h2><Link href="/uyelik" className="mt-8 inline-flex rounded-full bg-[#EC1C24] px-7 py-3 font-bold text-white">Üyelik başvurusu yap</Link></div>
      </section>
    </>
  );
}
