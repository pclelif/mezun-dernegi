...
export async function getEventBySlug(slug: string) {
  const fallback = FALLBACK_EVENTS.find((e) => e.slug === slug) || null;
  if (!isSupabaseConfigured()) return fallback;
  return withTimeout(async () => {
    const supabase = createServerAnonClient();
...
export async function getAnnouncementBySlug(slug: string) {
  const fallback = FALLBACK_ANNOUNCEMENTS.find((a) => a.slug === slug) || null;
  if (!isSupabaseConfigured()) return fallback;
  return withTimeout(async () => {
    const supabase = createServerAnonClient();
...
export async function getGalleryBySlug(slug: string) {
  const fallback = FALLBACK_GALLERIES.find((g) => g.slug === slug) || null;
  if (!isSupabaseConfigured()) return fallback;
  return withTimeout(async () => {
    const supabase = createServerAnonClient();
